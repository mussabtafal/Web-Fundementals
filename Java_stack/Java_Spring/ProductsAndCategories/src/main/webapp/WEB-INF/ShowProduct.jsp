...
<body>
	<div class = "container-md-fluid m-5 p-5">
		<div class = "row justify-content-between">
			<div class="col-5">
				<h1>Product: <c:out value="${pro.name}"></c:out></h1>
				<h2>Categories</h2>
				<ul>
					<c:forEach var="product" items="${pro.categories}">
						<li><c:out value="${product.name}"></c:out></li>
					</c:forEach>
				</ul>
			</div>
			<div class = "col-2">
				<form action="/products/${pro.id}/relate">
					<select name="category" class="form-select form-select-lg mb-3" aria-label=".form-select-lg example">
					  <c:forEach var="category" items="${allowedCat}">
